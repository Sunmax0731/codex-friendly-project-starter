...
function runVsCodeExtensionGate() {
  ...
  const checks = [
    ...
    check('work-dashboard-polish', webview.includes('<details class="section" open>') && webview.includes('tag-priority-p0') && webview.includes('初回セットアップ'), 'dashboard has collapsible sections, tag classes, and setup grouping'),
    ...
    check('codex-exec-terminal', extension.includes('buildCodexExecScript') && extension.includes('writeLauncherFile') && codexCli.includes('@codexArgs') && codexCli.includes('$OutputEncoding') && codexCli.includes('$codexToolPathCandidates') && codexCli.includes('Get-Command rg.exe') && codexCli.includes('Get-Command gh.exe'), 'Codex exec UTF-8 launcher and tool PATH bootstrap command exists'),
    check('codex-run-options', extension.includes('pickCodexRunOptions') && extension.includes('codexReasoningEffort') && workItemStart.includes('Codex 実行設定'), 'Work Item Codex model and intelligence selection exists'),
