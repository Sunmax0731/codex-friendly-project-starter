...
test('isAgentDocPath recognizes root AI agent files and selected docs', () => {
  ...
  assert.equal(isAgentDocPath('D:/repo/agents/phases/01-requirements/AGENTS.md'), true);
  assert.equal(isAgentDocPath('D:/repo/skills/work-types/ui-ux/SKILL.md'), true);
  assert.equal(isAgentDocPath('D:/repo/Tasks/README.md'), false);
  assert.equal(isAgentDocPath('D:/repo/src/index.js'), false);
});

test('classifyAgentDoc gives core files the highest priority and groups development docs', () => {
  assert.equal(classifyAgentDoc('D:/repo/AGENTS.md').priority, 0);
  assert.equal(classifyAgentDoc('D:/repo/AGENTS.md').group, 'agent-control');
  ...
  assert.equal(classifyAgentDoc('D:/repo/docs/manual-test.md').group, 'development-docs');
  assert.equal(classifyAgentDoc('D:/repo/agents/phases/01-requirements/AGENTS.md').group, 'agent-control');
  assert.equal(classifyAgentDoc('D:/repo/skills/work-types/ui-ux/SKILL.md').group, 'agent-control');
});
...
test('scanAgentDocs walks a workspace and skips node_modules', async () => {
  ...
  fs.writeFileSync(path.join(root, 'agents', 'phases', '01-requirements', 'AGENTS.md'), '# Child AGENTS\n');
  fs.writeFileSync(path.join(root, 'node_modules', 'pkg', 'SKILL.md'), '# ignored\n');
  const docs = await scanAgentDocs(root);
  assert.deepEqual(docs.map((doc) => doc.relativePath), ['AGENTS.md', 'docs/design.md', 'agents/phases/01-requirements/AGENTS.md']);
});
