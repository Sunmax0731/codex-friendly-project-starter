...
function buildFirstPrompt(input = {}) {
  ...
  const lines = [
    ...
    '- 作業実行は VS Code Codex / Codex CLI 相当のローカル workspace agent として進め、VS Code の Explorer、Source Control、Codex panel の文脈を優先してください。',
    '- Codex Desktop 固有の操作を前提にせず、VS Code で開いている workspace と確認可能な検証結果を根拠にしてください。',
