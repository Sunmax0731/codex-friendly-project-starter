...
function buildCodexWorkItemDraftPrompt(input = {}, options = {}) {
  ...
  return [
    ...
    '  "phase": "00-inbox | 01-requirements | 02-specification | 03-design | 04-implementation | 05-test | 06-release | 07-maintenance",',
    ...
    '## 判定ルール',
    '',
    '- 既存の GUI 入力が明示指定されている場合は尊重し、自然言語から不足分だけ補完してください。',
    '- `00-inbox` は明示的に未整理またはあとで分類と書かれている場合だけ使い、判断可能な場合は各工程へ振り分けてください。',
    '- `確認`、`検証`、`テスト`、`整合` は phase `05-test` を優先します。',
    '- `表示`、`分類`、`振り分け`、`Dashboard`、`WebView`、`Composer`、`UI`、`UX` は phase `03-design` を優先します。',
