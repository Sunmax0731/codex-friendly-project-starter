...
Allowed ZIP paths are `docs/**`, `prompts/**`, `.codexflow/**`, `AGENTS.md`, and `README.codexflow.md`. Runtime-local `.codexflow/logs/**`, `.codexflow/run-prompts/**`, and `.codexflow/backups/**` are skipped. Code or executable paths such as `src/**`, `extension.js`, `package.json`, `package-lock.json`, `node_modules/**`, `.git/**`, `.github/**`, `.vscode/**`, `tests/**`, `out/**`, `dist/**`, `*.vsix`, `*.exe`, `*.dll`, `*.bat`, `*.cmd`, `*.ps1`, and `*.sh` are rejected.

Runtime output paths declared in `.codexflow/flow.json` are also validated. Handoff outputs are limited to `docs/handoff/**`: `flow.handoff.latest`, `flow.handoff.template`, and `phase.handoffPath` cannot point to code paths such as `src/**`, `extension.js`, `package.json`, or `tests/**`. Log outputs are limited to `.codexflow/logs/**`: `flow.logs.directory` and `phase.logPath` cannot point to `src/**`, `docs/**`, `prompts/**`, or other code/document roots. `flow.handoff.directory` may be `docs/handoff` or a child under it; `flow.logs.directory` may be `.codexflow/logs` or a child under it. `phase.logPath` must use a child under `.codexflow/logs/**`, so `.codexflow/logs` alone is rejected for phase artifacts. Empty runtime output fields in `flow.json` are treated as unset and fall back to safe defaults under `docs/handoff/**` or `.codexflow/logs/<phase-id>`. Relative traversal, absolute paths, drive paths, UNC paths, home-relative paths, null bytes, and `.` are invalid.

Phase `checks` are imported as part of `flow.json` and may be executed later by `Run Next Codex Flow Phase` or `Run All Codex Flow Phases`. Validate and Import report them as a warning/report detail, but they do not run during validation or import.
