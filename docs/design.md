...
Runtime output path validation is separate from ZIP entry validation and uses the same policy during package Validate/Import and normal runner execution. `flow.handoff.latest`, `flow.handoff.template`, and `phase.handoffPath` must resolve under `docs/handoff/**`. `flow.handoff.directory` must be `docs/handoff` or a child of it. `flow.logs.directory` must be `.codexflow/logs` or a child of it, while `phase.logPath` must resolve to a child under `.codexflow/logs/**` and cannot be `.codexflow/logs` alone. Empty runtime output fields in `flow.json` are treated as unset and normalize to safe defaults under `docs/handoff/**` or `.codexflow/logs/<phase-id>`. Non-empty runtime output paths reject `.`, `..` segments, absolute paths, Windows drive paths, UNC paths, `~/` paths, null bytes, and anything that resolves outside the workspace root.

Phase `checks` remain imported metadata and are shown in the validation report. Validate/Import never executes them; the background runner may execute them later when the user explicitly starts `Run Next Codex Flow Phase` or `Run All Codex Flow Phases`.
