...
Runtime output path validation is separate from ZIP entry validation and uses the same policy during package Validate/Import and normal runner execution. `flow.handoff.latest`, `flow.handoff.template`, and `phase.handoffPath` must resolve under `docs/handoff/**`. `flow.handoff.directory` must be `docs/handoff` or a child of it. `flow.logs.directory` and `phase.logPath` must resolve under `.codexflow/logs/**`. Runtime output paths reject empty strings, `.`, `..` segments, absolute paths, Windows drive paths, UNC paths, `~/` paths, null bytes, and anything that resolves outside the workspace root.
